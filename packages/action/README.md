...
- Requires every newly authorised clean artifact to have been created no later than the originating marker's persisted `maxWaitDeadlineAt`, or its legacy deadline derived from `headStartedAt`/`createdAt` and the current maximum wait; reconciliation may finish after the deadline, but an artifact first created after it cannot pass or reassert the gate.
