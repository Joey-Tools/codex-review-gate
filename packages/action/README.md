...
- Ancestor checks validate the documented REST commit-comparison fields and their closed relationship/count matrix against the exact 40-hex `base...head` request. They ignore undocumented `head_commit`, perform no extra head-commit GET, and fail closed on any schema or relationship contradiction.
