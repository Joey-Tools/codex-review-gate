...
- Ancestor checks 会对照 exact 40-hex `base...head` request，校验 documented REST commit-comparison fields 及其 closed relationship/count matrix。它们忽略 undocumented `head_commit`，不会额外 GET head commit，并对任何 schema 或 relationship 矛盾 fail closed。
