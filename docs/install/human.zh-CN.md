...
feature head、而 embedded PR entry 另行绑定 current default-branch repository/ref/SHA 的原因。POST
只能提交一次；HTTP 或 transport 结果若不能证明公开的 `201` response，则属于 inconclusive，不得 replay。
