...
- Active, closed-wait, failed-findings, and passed-history authorization all require the selected clean artifact to be created no later than the trusted live marker's persisted `maxWaitDeadlineAt`, or a legacy deadline derived from that live marker's `headStartedAt`/`createdAt` and the current maximum wait; persisted live wait-budget fields must match sticky state, an already-recorded legacy state deadline may only narrow the derived window, and a pre-deadline artifact may still finish reconciliation after that deadline.
...
- The trusted-live deadline regressions reject sticky-state deadline extension, derive legacy cutoffs from live marker time, and retain an earlier recorded legacy deadline across later maximum-wait increases.
