...
- Ancestry validation now binds the exact 40-hex `base...head` request, validates the documented response fields and counts, and applies a closed `ahead` / `identical` / `behind` / `diverged` relationship matrix.
...
- Runner regressions cover live-shaped valid `ahead`, valid non-ancestor `behind` and `diverged`, plus deterministic schema, count, merge-base, and relationship contradictions.
